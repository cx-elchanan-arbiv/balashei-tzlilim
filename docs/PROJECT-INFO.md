...
## 🔇 קול — סטטוס נוכחי: **סאונד בלבד, בלי דיבור**
- **החלטה (2026-07-05):** **אין TTS כלל** — לא הקראת-מילים, לא משוב מדובר, לא רמזים מדוברים. רק **צלילי Web Audio** (הצלחה מגוונת / טעות מזמינה / מנגינת-סיום) + חוויה ויזואלית + רמז *כתוב* שקט.
- מימוש: `speak()` הוא no-op (early return); כפתורי `.speak-btn` ובורר-הקול `.voicebar` מוסתרים ב-CSS.
- **סטודיו הקלטות** קיים בקוד אך **מוסתר** (להחזרה: כפתור `#studioBtn` ב-`renderHome`).
- מסלולים פתוחים לעתיד (אם נרצה קול): הקלטה-עצמית · Google he-IL Wavenet · Azure הילה/אברי.
